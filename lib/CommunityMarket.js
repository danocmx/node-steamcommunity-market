...
class CommunityMarket {
	...
	getListings(params) {
		...
		const desiredCount = CommunityMarket.getDesiredCount(listingParams);

		return this.fetchListings({
			url: CommunityMarket.getListingsUrl(params),
			qs: { ...listingParams, count: desiredCount ? 100 : listingParams.count },
			desiredCount,
		})
	 ...
	 */
	static getDesiredCount({ start, count }) {
		let desiredCount = 0;

		if (count > 100) {
			desiredCount = parseInt(count);
			desiredCount += start || 0;
		} else if (!count) {
			desiredCount = Infinity;
		}

		return desiredCount;
	}

	fetchListings({ url, qs, desiredCount }, lastResponse) {
		return this.requestAgent({
			url,
			qs,
			json: true,
			...
			.then((listings) => {
				...
				} else lastResponse = listings;

				const fetchMoreInfo = {
					totalCount,
					desiredCount,
					currentCount: Object.keys(lastResponse.listinginfo).length,
				};
	...
	search(params = {}) {
		...
		const desiredCount = CommunityMarket.getDesiredCount(searchParams);

		return this.fetchSearch({
			url: '/search/render',
			qs: { ...searchParams, count: desiredCount ? 100 : searchParams.count },
			desiredCount,
		})
	...
	fetchSearch({ url, qs, desiredCount }, lastResponse) {
		return this.requestAgent({
			url,
			qs,
			gzip: true,
			...
			.then((search) => {
				...
				} else lastResponse = results;

				const fetchMoreInfo = {
					totalCount,
					desiredCount,
					currentCount: lastResponse.length,
				};
