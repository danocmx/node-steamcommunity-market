...
class CommunityMarket {
	...
	getHistogram(params) {
		...
		return this.requestAgent({
			uri: `${CommunityMarket.url}/itemordershistogram`,
			qs: histogramParams,
			json: true,
			gzip: true,
		})
	...
	static getListingsUrl({ appid, marketHashName }) {
		return `${CommunityMarket.url}/listings/${appid}/${encodeURIComponent(marketHashName)}/render`;
	}
	...
	getOverview(params) {
		...
		return this.requestAgent({
			url: `${CommunityMarket.url}/priceoverview`,
			qs: overviewParams,
			json: true,
			gzip: true,
		})
	...
	search(params) {
		...
		return this.fetchSearch({
			url: `${CommunityMarket.url}/search/render`,
	...
	getPriceHistory(params) {
		...
		return this.requestAgent({
			url: `${CommunityMarket.url}/pricehistory`,
			qs: historyParams,
			json: true,
			gzip: true,
		})
